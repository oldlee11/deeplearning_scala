...
/**
 * 并行化函数是        卷积  covon2d_parallel(x,kernel,convn_type)   
 *           相关  cor2d_parallel(x,kernel,cor_type)
 * **/
...
object convn {
  ...
  def convn2d_parallel(x1_in:Array[Array[Double]],x2_in:Array[Array[Double]],type_in:String):Array[Array[Double]]={
    ...
    if(type_in=="full"){
      val rows_3:Int=rows_1+rows_2-1
      val cols_3:Int=cols_1+cols_2-1
      val result_row_index= (0 until rows_3).par
      val result_col_index= (0 until cols_3).par
      val rows_2_index= (0 until rows_2).par
      val cols_2_index= (0 until cols_2).par
      /*
      var result:Array[Array[Double]]=Array.ofDim[Double](rows_3,cols_3)
      for(i<-0 until result.length){
        for(j<-0 until result(0).length){
      ...
      result
      */
      result_row_index.map(i=>result_col_index.map(j=>rows_2_index.map(k=>cols_2_index.map(m=>if((i-k)>=0 && (i-k)<rows_1 && (j-m)>=0 && (j-m)<cols_1) x1_in(i-k)(j-m)*x2_in(k)(m) else 0).reduce(_+_)).reduce(_+_)).toArray).toArray 
    }else if(type_in=="valid"){
      if((rows_1-rows_2+1<0)||(cols_1-cols_2+1<0)){
        Array()
      }else{
        ...
        /*
        var result:Array[Array[Double]]=Array.ofDim[Double](rows_3,cols_3)
        for(i<-0 until rows_3){
          for(j<-0 until cols_3){
            for(k<-0 until rows_2){
              for(m<-0 until cols_2){
                result(i)(j) +=x1_in(i+k)(j+m)*x2_in(rows_2-1-k)(cols_2-1-m)    
              }
        ...
        result
        */
        result_row_index.map(i=>result_col_index.map(j=>rows_2_index.map(k=>cols_2_index.map(m=>x1_in(i+k)(j+m)*x2_in(rows_2-1-k)(cols_2-1-m)).reduce(_+_)).reduce(_+_)).toArray).toArray;
      }
    }else{
      Array()
    }
  ...
  def cor2d_parallel(x1_in:Array[Array[Double]],x2_in:Array[Array[Double]],type_in:String):Array[Array[Double]]={
    val rows_1:Int=x1_in.length    //行数码=hight
    val cols_1:Int=x1_in(0).length //列数据=width
    val rows_2:Int=x2_in.length    //行数码=hight
    val cols_2:Int=x2_in(0).length //列数据=width   
    if(type_in=="full"){
      ...
      result
      */
      result_row_index.map(i=>result_col_index.map(j=>rows_2_index.map(k=>cols_2_index.map(m=>if((i-k)>=0 && (i-k)<rows_1 && (j-m)>=0 && (j-m)<cols_1) x1_in(i-k)(j-m)*x2_in(rows_2-1-k)(cols_2-1-m) else 0).reduce(_+_)).reduce(_+_)).toArray).toArray 
    }else if(type_in=="valid"){
      if((rows_1-rows_2+1<0)||(cols_1-cols_2+1<0)){
        Array()
      }else{
        ...
        result
        */
        result_row_index.map(i=>result_col_index.map(j=>rows_2_index.map(k=>cols_2_index.map(m=>x1_in(i+k)(j+m)*x2_in(k)(m)).reduce(_+_)).reduce(_+_)).toArray).toArray;
      }
    }else{
      Array()
    }
...
*/    
    ...
    print("test convn2d_parallel('full')\n")
    convn2d_parallel(x1,x2,"full").foreach(x=>{x.foreach(x=>print(x+"\t"));print("\n")})
 ...
 * */
    ...
    print("test convn2d_parallel('valid')\n")
    convn2d_parallel(x1,x2,"valid").foreach(x=>{x.foreach(x=>print(x+"\t"));print("\n")})    
 ...
 * */    
    ...
    print("test cor2d_parallel('full')\n")
    cor2d_parallel(x1,x2,"full").foreach(x=>{x.foreach(x=>print(x+"\t"));print("\n")})
 ...
 * */
    ...
    print("test cor2d_parallel('valid')\n")
    cor2d_parallel(x1,x2,"valid").foreach(x=>{x.foreach(x=>print(x+"\t"));print("\n")})    
  }
}